...
function stacksFromRecord(record: Record<string, number> | Partial<Record<string, number>>, excludedIds: readonly string[] = []): CombatStackInput[] {
  return Object.entries(record).flatMap(([entityId, count]) => typeof count === 'number' && Number.isInteger(count) && count > 0
    ...
    ? [{ entityId: entityId as CombatEntityId, count, level: 0 }]
    : []);
}
...
export function createSimulatorScenarioFromSpyReport(state: SaveState, report: SpyReportSnapshot): SimulatorScenario {
  ...
  const defenderTech = createDefaultCombatTechnologies();
  const attackerCommanderStacks = stacksFromRecord(attackerCommanders);
  ...
  return {
    ...
    attacker: {
      ...
      ships: stacksFromRecord(attackerShips),
    ...
    defender: {
      ...
      ships: stacksFromRecord(report.fleet ?? {}, ['spy-probe']),

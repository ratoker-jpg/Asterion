...
function replaceStackCount(
  ...
  if (side === 'attacker') {
    ...
    const activeId = category === 'commanders'
      ? (() => {
          const nextCommanders = next as CombatStackInput[];
          const explicit = scenario.attacker.activeCommanderId;
          if (explicit && nextCommanders.some((stack) => stack.entityId === explicit && stack.count > 0)) return explicit;
          return nextCommanders.length === 1 ? nextCommanders[0]!.entityId as CommanderId : null;
        })()
      : undefined;
    return {
      ...scenario,
  ...
  const activeId = category === 'commanders'
    ? (() => {
        const nextCommanders = next as CombatStackInput[];
        const explicit = scenario.defender.activeCommanderId;
        if (explicit && nextCommanders.some((stack) => stack.entityId === explicit && stack.count > 0)) return explicit;
        return nextCommanders.length === 1 ? nextCommanders[0]!.entityId as CommanderId : null;
      })()
    : undefined;
...
function maxForEntity(
  ...
  const ownedMax = entity.maxOwned ?? (category === 'defenses' && UNIQUE_DEFENSE_IDS.has(entity.id) ? 1 : Number.MAX_SAFE_INTEGER);
          ...
          return (
            <div className="sim-tech-row-v1" key={technology.id} data-qa-simulator-technology={technology.id}>
              ...
              <div>
                ...
                <small>Уровень сохраняется в профиле боя</small>
              </div>
...
function CommanderSelector({
  ...
  const selectedLabel = selected.length === 0
    ...
    : selected.length === 1
      ? active
        ? `Выбран: ${COMMANDER_COMBAT_CATALOG.find((entity) => entity.id === active)?.name ?? 'выбранный корабль'}`
        : 'Нет ведущего (авто-приоритет)'
      : active
        ? `Выбран: ${COMMANDER_COMBAT_CATALOG.find((entity) => entity.id === active)?.name ?? 'выбранный корабль'}`
        : 'Нет ведущего (авто-приоритет)';
  return (
    <label className="sim-commander-select-v1" htmlFor={`sim-leading-commander-${side}`}>
      ...
      <small id={`sim-leading-commander-help-${side}`}>Можно выбрать несколько разных типов, но по одному кораблю каждого типа. Здесь выбирается только ведущий; снятие ведущего не убирает корабли.</small>
    </label>
  );
}
...
export function SimulatorView({ planetName, coords, onBack }: { planetName: string; coords: string; onBack: () => void }) {
  ...
  const changeActiveCommander = (side: ScenarioSide, commanderId: CommanderId | null) => {
    updateScenario((current) => {
      ...
      let commanders = currentCommanders;
      if (commanderId && !currentCommanders.some((stack) => stack.entityId === commanderId && stack.count > 0)) {
        ...
        commanders = [...currentCommanders, { entityId: commanderId, count: 1, level: 0 }];
      }
      const nextActive = commanderId && commanders.some((stack) => stack.entityId === commanderId && stack.count > 0) ? commanderId : null;
    ...
    if (commanderId) setNotice('Ведущий командир выбран. Остальные выбранные командиры сохранены.');
  };

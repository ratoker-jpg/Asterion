...
| Role | Token | Size | Use |
| --- | --- | ---: | --- |
| Large title | `--ut-font-page-title` | 32px | Screen and modal titles |
| Section title | `--ut-font-section-title` | 18px | Panel, card, and mini-headings |
| Data / lead | `--ut-font-data` | 16px | Values, counters, table data |
| Control | `--ut-font-control` | 16px | Buttons, tabs, selects, form controls |
| Body | `--ut-font-body` | 14px | Descriptions and operational copy |
| Secondary | `--ut-font-secondary` | 14px | Labels, metadata, coordinates, status copy |
| Helper | `--ut-font-helper` | 14px | Hints and explanatory text |
| HUD | `--ut-font-hud` | 12px | Compact fixed header telemetry only |

New content must not introduce 6–11px text. A smaller HUD value is an existing fixed-chrome exception, not a default for panels, cards, operations, settings, or actions.
...
```text
32  ASTERION / РЕЙТИНГ
18  ОПЕРАЦИОННЫЙ ЦЕНТР
16  1 325 120        ОТПРАВИТЬ        РЕСУРСНЫЕ ОЧКИ
14  Проверка цели и создание планеты.  ГАЛАКТИКА 1 · СИСТЕМА 07
```
...
- Start ordinary copy at 14px and headings at 18px or 32px according to role.

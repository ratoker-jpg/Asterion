...
Источник механических характеристик для этого changeset — сохранённые HTML-страницы публичного репозитория `ratoker-jpg/Nemexia_auto_v2`, commit `a5aa72739113a328be104d761cd664cb4d0a7853`:
...
- `Время` из HTML используется как provenance/контрольное исходное значение, а runtime `construction.time` берётся из `ASTERION_BALANCE_V1_TIME_REBALANCED/корабли/{Астеры,Илары,Рой}/00_TIME_REBALANCED.md`, колонка `Новое 2%/шт.`; коэффициенты ускорения применяются только расчётами runtime/preview.
...
- `solar-satellite`: у всех трёх рас совпадают стоимость, population, атака, жизнь, Time Rebalanced-время и требование верфи; тип брони расовый, а transport-трейт отсутствует у всех трёх страниц.

Остальные расхождения намеренно сохранены в `FACTION_SHIP_MECHANICS`; действующие формулы боевого расчёта, стоимости улучшений, бонусы космодрома и промышленной фабрики не изменялись.

## Factory upgrades (follow-up)

Источник таблиц улучшений — `D:/Desktop/Asterion/ASTERION_BALANCE_V1/корабли`. Подключены только секции `Factory upgrades`: 9 таблиц × 10 переходов для каждой фракции, то есть 27 таблиц всего:

- `Астеры` → `aegis`;
- `Илары` → `synod`;
- `Рой` → `veyra`.

В `src/domain/buildings/spaceport-upgrade-balance-v1.ts` сохранены канонические значения стоимости (металл/минералы/газ) из `Factory upgrades`; только duration заменён на соответствующую строку `Время 2%` из `ASTERION_BALANCE_V1_TIME_REBALANCED/корабли/{Астеры,Илары,Рой}/00_TIME_REBALANCED.md`. Опубликованная строка `Level 1` хранится в индексе `0` и означает переход `0 → 1`; строка `Level N` хранится в индексе `N - 1`. Preview передаёт текущий проектируемый уровень, поэтому смещения на один уровень нет. Общая таблица Астеров не используется как fallback для Иларов или Роя.

У четырёх служебных кораблей (`solar-satellite`, `spy-probe`, `recycler`, `colonizer`) в исходных MD нет `Factory upgrades`; они по-прежнему исключены из каталога улучшений Космодрома. Базовая стоимость, population, характеристики и требования кораблей остаются из HTML-реестра; базовое время производства — отдельная колонка `Новое 2%/шт.`. Уже созданные задания очереди используют сохранённые `effectiveDurationMs`/timestamps и не пересчитываются.

## Defense construction follow-up

Для всех 9 типов обороны × 3 фракции подключены отдельные строки в `src/domain/combat/defense-construction-data.ts`:

- стоимость и population — из `ASTERION_BALANCE_V1/оборона/{Астеры,Илары,Рой}/*.md`, таблица `Полные характеристики источника`;
- базовое время — из `ASTERION_BALANCE_V1_TIME_REBALANCED/оборона/{Астеры,Илары,Рой}/00_TIME_REBALANCED.md`, колонка `Базовое время 1,4%`.

`getFactionDefenseCatalog(factionId)` теперь отдаёт эту механику для выбранной расы, а `ConstructionCatalogView` использует именно этот каталог. Боевые поля, роли, требования, названия и assets не заменялись; tower/planetary shield сохраняют лимит одной копии.

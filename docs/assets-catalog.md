...
## Asset integration — 25.09.2026

| Группа | Файлов | Использование |
|---|---:|---|
| Иконки Reports/Profile | 8 | существующие папки сообщений и профиль |
| Иконки очков | 4 | метрики ресурсов, боя, общих очков и достижений; общие очки используют звезду |
| Портреты генералов | 3 | совпадают с уже используемыми исходниками и переиспользуются |
| Pirate Warlord | 1 | исходный арт на будущее |
| Pirate ships | 7 | исходный арт на будущее, не подключён к флоту или бою |
| Regular planet skins | 9 | выбираемые стабильные ID `skin-asterion-01`…`skin-asterion-09` |
| Pirate planet art | 10 | варианты существующих объектов «Отступники» |
| Unique object art | 16 | изображения существующих уникальных объектов |
| Anomalies | 10 | новый набор вместо двух удалённых изображений |
| Battle faction icons | 4 | квадратные иконки для слотов боевых докладов; Pirates пока не отображается |

В репозиторий добавлены 69 исходных файлов и переиспользованы 3 портрета генералов. Runtime-версии и проверки лимитов формируются скриптом [`asterion-asset-integration.mjs`](../tools/asterion-asset-integration.mjs); подробности перечислены в [integration manifest](../assets/manifests/asterion-asset-integration.manifest.json) и [asset audit](../assets/manifests/asterion-asset-integration.audit.json). Альтернативная иконка общих очков не включена.
...
В `stellar-remnants` находятся **10 подготовленных PNG** из набора интеграции от 25.09.2026. Runtime использует их оптимизированные WebP. Два прежних fallback-файла удалены после замены ссылок; типы объектов, вероятность появления и поведение не менялись. Пиратские корабли и Pirate Warlord остаются source-only материалами на будущее.
...
### 2. Разные ассеты аномалий

Текущая привязка `anomalyArts` закрывает только fallback-слой. До подключения нового набора не следует маскировать нехватку перекраской одного и того же ремнанта: у каждого типа аномалии должны быть свой asset ID, силуэт, палитра и короткое описание без выдуманных наград или эффектов.

### 3. Финальный выбор дублирующихся faction arts

...
### Пробел: разнообразные аномалии

В каталоге сейчас есть только **2 `stellar-remnants` PNG**. Они годятся как временный fallback, но визуально читаются как один и тот же светящийся объект и не образуют полноценный набор аномалий. Для экрана Вселенной нужен отдельный anomaly pack минимум из **8 разных силуэтов**:

- гравитационная линза / кольцо Эйнштейна;
- аккреционный диск чёрной дыры с релятивистскими джетами;
- магнитар или пульсарная туманность;
- оболочка остатка сверхновой;
- тёмная туманность / Bok globule;
- протозвезда с биполярным Herbig–Haro выбросом;
- парные космические пузыри Fermi-типа;
- вспышка с асимметричным потоком после слияния компактных объектов.

Требования к генерации и точные имена файлов находятся в [`docs/prompts/universe-art-v2.md`](./prompts/universe-art-v2.md), раздел **Anomaly discovery pack**. Спекулятивные червоточины, космические струны и мегаструктуры следует добавлять отдельными редкими типами и явно маркировать как гипотетические.
